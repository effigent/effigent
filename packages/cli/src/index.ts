...
program.name('effigent').description('effigent — the Optimizer CLI: capture agent runs, compile away the waste').version('0.3.0');
...
program
  ...
  .action(async (agentName: string, opts) => {
    ...
    console.log(`✓ bundle written: ${join(outDir, 'bundle.json')}`);
